...
window.onload = function() {
  ...
  data.forEach((student, i) => {
    ...
    console.log(_website, "web");
    console.log(_linkedin, "ln");
  ...
  let searchFunction = document.querySelector(".form-control");
  searchFunction.addEventListener("input", function(event) {
    let searchValue = event.target.value.toLowerCase();
    data.forEach((student, i) => {
      let student_id = document.getElementById(`student_${i}`);

      let isVisible = () => {
        if (student.basic_info.first_name && student.basic_info.last_name) {
          return (
            student.basic_info.first_name.toLowerCase().includes(searchValue) ||
            student.basic_info.last_name.toLowerCase().includes(searchValue)
          );
        }
      };
      ...
      student_id.classList.toggle("d-none", !isVisible());
    });
  });
};
