...
const node = data.resumesYaml;
const [isBuilt, setIsBuilt] = useState(false);
useEffect(()=> {
  setIsBuilt(true)
}, [])
return( <>
    {isBuilt && <PDFViewer width={1500} height={1500}>{() => 
         <Document>
         <Page style={styles.body}>
           <View style={styles.dflex}>
             <View style={styles.studenName}>
               <Text style={styles.name}>{node.basic_info.first_name !== undefined ? stripName(node.basic_info.first_name.toUpperCase()): "FIRST NAME"}</Text>
               <Text style={styles.lastname}>{node.basic_info.last_name !== undefined ? stripName(node.basic_info.last_name.toUpperCase()): "LAST NAME"}</Text>
             ...
             <View style={styles.contactInfoView}>
              <View style={styles.dflexInfo}>
               ...
               <Text style={styles.contactInfoText}>{node.basic_info.github !== undefined ? node.basic_info.github : "Your Github"}</Text>
               </View>
               <View style={styles.dflexInfo}>
               <Image src='https://cdn4.iconfinder.com/data/icons/squared-line-communication/64/mail-circle-512.png' style={styles.img}/>
               <Text style={styles.contactInfoText}>{node.basic_info.email !== undefined ? node.basic_info.email: "Your Email"}</Text>
               </View>
               <View style={styles.dflexInfo}>
               <Image src='https://cdn2.iconfinder.com/data/icons/font-awesome/1792/phone-512.png' style={styles.img}/>
               <Text style={styles.contactInfoText}> { node.basic_info.phone !== undefined ?  node.basic_info.phone : "Your Phone" }</Text>
               </View>
               <View style={styles.dflexInfo}>
               <Image src='https://cdn.onlinewebfonts.com/svg/img_408253.png' style={styles.img}/>
                <Text style={styles.contactInfoText}>{node.basic_info.linkedin !== undefined ? node.basic_info.linkedin : "Your Linkedin"}</Text>
               </View>
             </View>
           </View>
           <View style={styles.pb}>
             <Text style={styles.titleB}>SUMMARY</Text>
             <Text style={styles.descriptionD}>{node.basic_info != null ? stripJumps(node.basic_info.summary) : "Amateur man in programming, with a desire to learn to use it in everyday life"}</Text>
           </View>
           <View style={styles.section}>
             <View style={styles.skills}>
               ...
               <View>
                 ...
                 {node.skills.toolset != null ? node.skills.toolset.map((item,i) => <Text style={styles.description} key={i}>{item.name + " - " + convertSkill(item.level)}</Text>) : <Text style={styles.description}>Skill - Intermediate</Text>}
               </View>
               <View>
                 <Text style={styles.subtitle}>PROJECTS</Text>
                {
                 node.projects.assignments != null ?
                ...
                <View key={i}>
                 ...
                 <Text style={styles.subtitle}>LANGUAGES</Text>
                  {node.basic_info.languages != null ? node.basic_info.languages.map((item, i) => <Text style={styles.description} key={i}>{item.idiom + " - " + item.level}</Text>) : <Text style={styles.description}>Language - Level</Text>}
                 </View>
               </View>
             <View style={styles.experience}>
               <Text style={styles.title}>EXPERIENCE</Text>
               {node.experiences != null ? node.experiences.map( (item, i) =>
               <View key={i}>
                 ...
                 <View>
                   {node.education != null ? node.education.map( (item, i) => 
