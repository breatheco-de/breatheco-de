...
export interface ActionFunction<TContext, TEvent extends EventObject> {
    (context: TContext, event: TEvent, meta: ExecMeta<TContext, TEvent>): any | void;
    name: string;
}
...
export declare type ConditionPredicate<TContext, TEvent extends EventObject> = (context: TContext, event: TEvent, microstepState: StateValue) => boolean;
export declare type Condition<TContext, TEvent extends EventObject> = string | ConditionPredicate<TContext, TEvent>;
 ...
 */
export declare type InvokeCreator<TFinalContext, TContext> = (context: TContext, event: EventObject) => Promise<TFinalContext> | StateMachine<TFinalContext, any, any> | InvokeCallback;
...
export declare type TransitionsConfig<TContext, TEvent extends EventObject> = {
    [K in TEvent['type'] | BuiltInEvent<TEvent>['type']]?: string | number | StateNode<TContext> | SingleOrArray<TransitionConfig<TContext, Extract<TEvent, {
        type: K;
    }>>>;
};
...
export declare type ServiceConfig<TContext> = string | StateMachine<any, any, any> | InvokeCreator<any, TContext>;
...
export declare type Assigner<TContext, TEvent extends EventObject> = (extState: TContext, event: TEvent) => Partial<TContext>;
...
export declare type Mapper<TContext, TEvent extends EventObject> = (extState: TContext, event: TEvent) => any;
