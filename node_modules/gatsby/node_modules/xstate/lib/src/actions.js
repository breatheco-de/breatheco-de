...
function resolveSend(action, ctx, event) {
    ...
    var resolvedEvent = typeof action.event === 'function'
        ? toEventObject(action.event(ctx, event))
        : toEventObject(action.event);
    var resolvedDelay = typeof action.delay === 'function'
        ? action.delay(ctx, event)
        : action.delay;
    return __assign({}, action, { event: resolvedEvent, delay: resolvedDelay });
}
