...
var StateNode = /** @class */ (function () {
    ...
    StateNode.prototype.getDelayedTransitions = function () {
        ...
        if (utils_1.isArray(afterConfig)) {
            return afterConfig.map(function (delayedTransition, i) {
                ...
                return __assign({ event: event }, delayedTransition, { cond: delayedTransition.cond
                        ? _this.toGuard(delayedTransition.cond)
                        : undefined, actions: utils_1.toArray(delayedTransition.actions).map(function (action) {
                        return actions_1.toActionObject(action);
                    }) });
            });
        }
        var allDelayedTransitions = utils_1.flatten(utils_1.keys(afterConfig).map(function (delayKey) {
            ...
            return delayedTransitions.map(function (transition) { return (__assign({ event: event,
                delay: delay }, transition, { cond: transition.cond ? _this.toGuard(transition.cond) : undefined, actions: utils_1.toArray(transition.actions).map(function (action) {
                    return actions_1.toActionObject(action);
                }) })); });
        }));
    ...
    StateNode.prototype.transitionLeafNode = function (stateValue, state, eventObject) {
        ...
        if (!next.tree) {
            var _a = this.next(state, eventObject), reentryStates = _a.reentryStates, actions = _a.actions, tree = _a.tree;
            return {
                tree: tree,
                source: state,
    ...
    StateNode.prototype.transitionParallelNode = function (stateValue, state, eventObject) {
        ...
        if (!willTransition) {
            var _d = this.next(state, eventObject), reentryStates = _d.reentryStates, actions = _d.actions, tree = _d.tree;
            return {
                tree: tree,
                source: state,
                reentryStates: reentryStates,
        ...
        if (allPaths.length === 1 &&
            ...
            return {
                ...
                reentryStates: utils_1.keys(transitionMap)
                    ...
                    try {
                        for (var reentryStates_1 = __values(reentryStates), reentryStates_1_1 = reentryStates_1.next(); !reentryStates_1_1.done; reentryStates_1_1 = reentryStates_1.next()) {
                            var reentryState = reentryStates_1_1.value;
                        ...
                        try {
                            if (reentryStates_1_1 && !reentryStates_1_1.done && (_a = reentryStates_1.return)) _a.call(reentryStates_1);
                        }
    ...
    StateNode.prototype.evaluateGuard = function (guard, context, eventObject, state) {
        ...
        if (!guards[guard.type]) {
            throw new Error("Guard (condition) '" + guard.type + "' is not implemented on machine '" + this.machine.id + "'.");
        }
        condFn = guards[guard.type];
        return condFn(context, eventObject, guardMeta);
    };
    StateNode.prototype.toGuard = function (condition) {
        if (utils_1.isString(condition)) {
            return {
                type: condition
            };
        }
        if (utils_1.isFunction(condition)) {
            return {
                type: 'xstate.cond',
                predicate: condition
            };
    ...
    StateNode.prototype.getActions = function (transition, prevState) {
        ...
        var actions = exitActions
            ...
            .concat(entryActions)
            .map(function (action) { return actions_1.toActionObject(action, _this.machine.options.actions); });
        return actions;
    };
    /**
    ...
    StateNode.prototype.resolveTransition = function (stateTransition, currentState, eventObject) {
        ...
        var historyValue = currentState.historyValue
            ...
            : stateTransition.source
                ...
                : undefined;
        if (!exports.IS_PRODUCTION && stateTransition.tree) {
            try {
        ...
        var activities = __assign({}, currentState.activities);
        try {
        ...
        var resolvedActions = nonEventActions.map(function (action) {
            var actionObject = actions_1.toActionObject(action);
            if (actionObject.type === actionTypes.send) {
                ...
                if (utils_1.isString(sendAction.delay)) {
                    if (!_this.machine.options.delays ||
                        ...
                        utils_1.warn(false, "No delay reference for delay expression '" + sendAction.delay + "' was found on machine '" + _this.machine.id + "'");
    ...
    StateNode.prototype.getInitialState = function (stateValue, context) {
        ...
        var actions = [];
        try {
            for (var _c = __values(this.getStateNodes(stateValue)), _d = _c.next(); !_d.done; _d = _c.next()) {
                var stateNode = _d.value;
                if (stateNode.onEntry) {
                    actions.push.apply(actions, __spread(stateNode.onEntry));
                }
                if (stateNode.activities) {
                    try {
                        for (var _e = __values(stateNode.activities), _f = _e.next(); !_f.done; _f = _e.next()) {
                            var activity = _f.value;
                            activityMap[utils_1.getEventType(activity)] = activity;
        ...
        var updatedContext = this.options.updater(context, { type: types_1.ActionTypes.Init }, assignActions);
        var initialNextState = new State_1.State({
            value: stateValue,
            context: updatedContext,
            event: actions_1.initEvent,
    ...
    Object.defineProperty(StateNode.prototype, "initialState", {
        ...
        get: function () {
            ...
            return this.resolveTransition({
                ...
                actions: []
            }, state, { type: types_1.ActionTypes.Init });
        },
        enumerable: true,
        configurable: true
    });
    ...
    StateNode.prototype.formatTransition = function (target, transitionConfig, event) {
        ...
        if (target === undefined || target === TARGETLESS_KEY) {
            return __assign({}, transitionConfig, { actions: transitionConfig
                    ...
                    ? transitionConfig.cond
                        ...
                        : undefined
                    : undefined, target: undefined, internal: internal === undefined ? true : internal, event: event });
        }
        ...
        var formattedTargets = targets.map(function (_target) {
            ...
            var isInternalTarget = utils_1.isString(_target) && _target[0] === _this.delimiter;
        ...
        return __assign({}, transitionConfig, { actions: transitionConfig
                ...
                ? transitionConfig.cond
                    ? this.toGuard(transitionConfig.cond)
                    : undefined
                : undefined, target: formattedTargets, internal: internal,
            event: event });
    };
    StateNode.prototype.formatTransitions = function () {
        ...
        var formattedTransitions = utils_1.mapValues(__assign({}, onConfig, doneConfig, invokeConfig), function (value, event) {
            ...
            if (utils_1.isString(value) || value instanceof StateNode) {
