...
const getFieldConfigFromFieldNameConvention = ({
  ...
  if (linkedTypes.length > 1) {
    ...
    type = schemaComposer.getOrCreateUTC(typeName, utc => {
      const types = linkedTypes.map(typeName => schemaComposer.getOrCreateOTC(typeName));
      utc.setTypes(types);
  ...
  switch (typeof value) {
    ...
    case `object`:
      ...
      /* && depth < MAX_DEPTH*/
          ...
          if (typeComposer.hasField(key) && getNamedType(typeComposer.getFieldType(key)) instanceof GraphQLObjectType) {
            const originalFieldTypeComposer = typeComposer.getFieldTC(key);
            fieldTypeComposer = originalFieldTypeComposer.clone(originalFieldTypeComposer.getTypeName());
          } else {
            fieldTypeComposer = ObjectTypeComposer.createTemp(createTypeName(selector), schemaComposer);
          }
...
const createFieldName = key => {
  ...
  const replaced = key.replace(NON_ALPHA_NUMERIC_EXPR, `_`); // key is invalid; normalize with leading underscore and rest with x
