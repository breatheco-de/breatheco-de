...
const addInferredTypes = ({
  ...
  const typesWithNodes = Object.keys(typeMap).filter(typeName => hasNodes(typeMap[typeName]));
...
const addInferredType = ({
  ...
  const typeName = typeComposer.getTypeName(); // TODO: Move this to where the type is created once we can get
  // node type owner information directly from store

  if (typeComposer.getExtension(`createdFrom`) === `inference` && hasNodes(inferenceMetadata.typeMap[typeName])) {
    const nodes = nodeStore.getNodesByType(typeName);
    typeComposer.setExtension(`plugin`, nodes[0].internal.owner);
  }

  const exampleValue = getExampleObject(Object.assign({}, inferenceMetadata.typeMap[typeName], {
    typeName,
    typeConflictReporter
  }));
  addInferredFields({
