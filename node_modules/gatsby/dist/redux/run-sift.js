...
const getFilters = filters => Object.keys(filters).map(key => {
  return {
    [key]: filters[key]
  };
}); /////////////////////////////////////////////////////////////////////
...
const runFlatFilterWithoutSift = (chain, targetValue, nodeTypeNames, typedKeyValueIndexes) => {
  ensureIndexByTypedChain(chain, nodeTypeNames, typedKeyValueIndexes);
  const nodesByKeyValue = getNodesByTypedChain(chain, targetValue, nodeTypeNames, typedKeyValueIndexes); // If we couldn't find the needle then maybe sift can, for example if the
...
const runFilterAndSort = args => {
  ...
  let result = applyFilters(filter, firstOnly, nodeTypeNames, typedKeyValueIndexes, resolvedFields);
...
const applyFilters = (filterFields, firstOnly, nodeTypeNames, typedKeyValueIndexes, resolvedFields) => {
  const filters = filterFields ? prefixResolvedFields(createDbQueriesFromObject(prepareQueryArgs(filterFields)), resolvedFields) : [];
...
const filterWithoutSift = (filters, nodeTypeNames, typedKeyValueIndexes) => {
  ...
  if (!typedKeyValueIndexes || filters.length !== 1 || filters[0].type === `elemMatch` || filters[0].query.comparator !== `$eq`) {
    return undefined;
  }

  const filter = filters[0];
  return runFlatFilterWithoutSift(filter.path, filter.query.value, nodeTypeNames, typedKeyValueIndexes);
}; // Not a public API
...
const filterWithSift = (filter, firstOnly, nodeTypeNames, resolvedFields) => {
  ...
  return _runSiftOnNodes(nodes, filter.map(f => dbQueryToSiftQuery(f)), firstOnly, nodeTypeNames, resolvedFields, siftGetNode);
};
/**
...
const runSiftOnNodes = (nodes, args, getNode = siftGetNode) => {
  ...
  let siftFilter = getFilters(liftResolvedFields(toDottedFields(prepareQueryArgs(filter)), resolvedFields));
  return _runSiftOnNodes(nodes, siftFilter, firstOnly, nodeTypeNames, resolvedFields, getNode);
};
...
const _runSiftOnNodes = (nodes, filter, firstOnly, nodeTypeNames, resolvedFields, getNode) => {
...
const sortNodes = (nodes, sort, resolvedFields) => {
  ...
  }).map(field => v => (0, _getValueAt.getValueAt)(v, field));
