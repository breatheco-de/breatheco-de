...
const ensureIndexByTypedChain = (chain, nodeTypeNames, typedKeyValueIndexes) => {
  const chained = chain.join(`+`);
  const nodeTypeNamePrefix = nodeTypeNames.join(`,`) + `/`; // The format of the typedKey is `type,type/path+to+eqobj`
...
const getNodesByTypedChain = (chain, value, nodeTypeNames, typedKeyValueIndexes) => {
  const key = chain.join(`+`);
  const typedKey = nodeTypeNames.join(`,`) + `/` + key;
  let byTypedKey = typedKeyValueIndexes === null || typedKeyValueIndexes === void 0 ? void 0 : typedKeyValueIndexes.get(typedKey);
