...
describe('SchemaComposer', () => {
  ...
  describe('addTypeDefs', () => {
    it('should parse types from SDL', () => {
      ...
      `);
      expect(sc.get('Author')).toBeInstanceOf(_graphql.GraphQLObjectType);
      expect(sc.get('AuthorInput')).toBeInstanceOf(_graphql.GraphQLInputObjectType);
      expect(sc.get('MyInt')).toBeInstanceOf(_graphql.GraphQLScalarType);
      expect(sc.get('Sort')).toBeInstanceOf(_graphql.GraphQLEnumType);
      expect(sc.get('PersonI')).toBeInstanceOf(_graphql.GraphQLInterfaceType);
    });
    it('should parse cross referenced types from SDL', () => {
      ...
      `);
      expect(sc.get('Author')).toBeInstanceOf(_graphql.GraphQLObjectType);
      expect(sc.get('Post')).toBeInstanceOf(_graphql.GraphQLObjectType); // Post type should be the same instance

      const Post = sc.get('Post');

      const PostInAuthor = _ObjectTypeComposer.ObjectTypeComposer.createTemp(sc.get('Author')).getFieldTC('posts').getType();

      expect(Post).toBe(PostInAuthor); // Author type should be the same instance

      const Author = sc.get('Author');

      const AuthorInPost = _ObjectTypeComposer.ObjectTypeComposer.createTemp(sc.get('Post')).getFieldTC('author').getType();
  ...
  describe('works with directives', () => {
    ...
    function removeDefaultDirectives(sc) {
      sc.removeDirective(_graphql.GraphQLSkipDirective);
      sc.removeDirective(_graphql.GraphQLIncludeDirective);
      sc.removeDirective(_graphql.GraphQLDeprecatedDirective);
    }
