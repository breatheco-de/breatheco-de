...
class SchemaComposer extends _TypeStorage.TypeStorage {
  constructor() {
    ...
    this.typeMapper = new _TypeMapper.TypeMapper(this); // alive proper Flow type casting in autosuggestions for class with Generics
  ...
  rootQuery() {
    (0, _deprecate.default)('Use schemaComposer.Query property instead');
    return this.getOrCreateOTC('Query');
  }
    ...
    tc.getFieldNames().forEach(fieldName => {
      ...
      if (fieldType instanceof _graphql.GraphQLObjectType) {
        ...
        if (!passedTypes.has(typeName)) {
          ...
          const fieldTC = new _ObjectTypeComposer.ObjectTypeComposer(fieldType, this);
  ...
  getTC(typeName) {
    (0, _deprecate.default)(`Use SchemaComposer.getOTC() method instead`);
    return this.getOTC(typeName);
  }
  ...
  addAsComposer(typeOrSDL) {
    ...
    if (type instanceof _ObjectTypeComposer.ObjectTypeComposer || type instanceof _InputTypeComposer.InputTypeComposer || type instanceof _ScalarTypeComposer.ScalarTypeComposer || type instanceof _EnumTypeComposer.EnumTypeComposer || type instanceof _InterfaceTypeComposer.InterfaceTypeComposer || type instanceof _UnionTypeComposer.UnionTypeComposer) {
      ...
      return _ObjectTypeComposer.ObjectTypeComposer.create(type, this).getTypeName();
    } else if (type instanceof _graphql.GraphQLInputObjectType) {
      return _InputTypeComposer.InputTypeComposer.create(type, this).getTypeName();
    } else if (type instanceof _graphql.GraphQLScalarType) {
      return _ScalarTypeComposer.ScalarTypeComposer.create(type, this).getTypeName();
    } else if (type instanceof _graphql.GraphQLEnumType) {
      return _EnumTypeComposer.EnumTypeComposer.create(type, this).getTypeName();
    } else if (type instanceof _graphql.GraphQLInterfaceType) {
      return _InterfaceTypeComposer.InterfaceTypeComposer.create(type, this).getTypeName();
    } else if (type instanceof _graphql.GraphQLUnionType) {
      return _UnionTypeComposer.UnionTypeComposer.create(type, this).getTypeName();
    }
  ...
  addTypeDefs(typeDefs) {
    ...
    if (types.has('Query')) {
      this.Query.addFields(_ObjectTypeComposer.ObjectTypeComposer.create(types.get('Query'), this).getFields());
    }

    if (types.has('Mutation')) {
      this.Mutation.addFields(_ObjectTypeComposer.ObjectTypeComposer.create(types.get('Mutation'), this).getFields());
    }

    if (types.has('Subscription')) {
      this.Subscription.addFields(_ObjectTypeComposer.ObjectTypeComposer.create(types.get('Subscription'), this).getFields());
    }
