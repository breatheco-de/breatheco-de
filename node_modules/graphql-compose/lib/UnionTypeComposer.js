...
class UnionTypeComposer {
  ...
  static createTemp(typeDef, schemaComposer) {
    ...
    if ((0, _is.isString)(typeDef)) {
      ...
      const type = new _graphql.GraphQLUnionType(_objectSpread({}, typeDef, {
        types: (0, _is.isFunction)(types) ? () => (0, _configAsThunk.resolveTypeArrayAsThunk)(sc, types(), typeDef.name) : () => []
      }));
      UTC = new UnionTypeComposer(type, sc);
  ...
  constructor(gqType, schemaComposer) {
    ...
    if (!(gqType instanceof _graphql.GraphQLUnionType)) {
  ...
  hasType(name) {
    const nameAsString = (0, _typeHelpers.getComposeTypeName)(name);
  ...
  _getTypeMap() {
    if (!this.gqType._gqcTypeMap) {
      ...
      if (_graphqlVersion.graphqlVersion >= 14) {
        this.gqType._types = () => {
          return (0, _configAsThunk.resolveTypeArrayAsThunk)(this.schemaComposer, this.getTypes(), this.getTypeName());
        };
      } else {
  ...
  setTypes(types) {
    this.clearTypes();
    types.forEach(type => {
      this._getTypeMap().set((0, _typeHelpers.getComposeTypeName)(type), type);
    });
    return this;
  }
