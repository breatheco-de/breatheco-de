...
import { graphql, GraphQLString, GraphQLObjectType, GraphQLInputObjectType, GraphQLInterfaceType, GraphQLEnumType, GraphQLUnionType, GraphQLDirective, DirectiveLocation, GraphQLSkipDirective, GraphQLIncludeDirective, GraphQLDeprecatedDirective, GraphQLScalarType } from '../graphql';
describe('SchemaComposer', () => {
  ...
  describe('addTypeDefs', () => {
    ...
    it('should parse cross referenced types from SDL', () => {
      ...
      `);
      expect(sc.get('Author')).toBeInstanceOf(GraphQLObjectType);
      expect(sc.get('Post')).toBeInstanceOf(GraphQLObjectType); // Post type should be the same instance

      const Post = sc.get('Post');
      const PostInAuthor = ObjectTypeComposer.createTemp(sc.get('Author')).getFieldTC('posts').getType();
      expect(Post).toBe(PostInAuthor); // Author type should be the same instance

      const Author = sc.get('Author');
      const AuthorInPost = ObjectTypeComposer.createTemp(sc.get('Post')).getFieldTC('author').getType();
  ...
  describe('addResolveMethods', () => {
    ...
    _asyncToGenerator(function* () {
      ...
      const schema = sc.buildSchema();
      expect((yield graphql(schema, '{ posts { id title votes } }'))).toEqual({
        data: {
          posts: [{
            id: 1,
            title: 'Post title',
