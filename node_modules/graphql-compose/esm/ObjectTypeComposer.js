...
import { GraphQLObjectType, GraphQLList, GraphQLNonNull, GraphQLInputObjectType, getNamedType, GraphQLInterfaceType, isType } from './graphql';
...
export function isComposeOutputType(type) {
  return isType(type) || Array.isArray(type) && isComposeOutputType(type[0]) || type instanceof Resolver || type instanceof ObjectTypeComposer || type instanceof InterfaceTypeComposer || type instanceof EnumTypeComposer || type instanceof UnionTypeComposer || type instanceof ScalarTypeComposer;
} // Compose Args -----------------------------
...
export class ObjectTypeComposer {
  ...
  getFieldTC(fieldName) {
    ...
    if (!(fieldType instanceof GraphQLObjectType)) {
      throw new Error(`Cannot get ObjectTypeComposer for field '${fieldName}' in type ${this.getTypeName()}. ` + `This field should be ObjectType, but it has type '${fieldType.constructor.name}'`);
    }
  ...
  getInterfaces() {
    if (!this.gqType._gqcInterfaces) {
      ...
      this.gqType._gqcInterfaces = resolveMaybeThunk(interfaces) || [];
    }
  ...
  setInterfaces(interfaces) {
    ...
    const interfacesThunk = () => {
      return interfaces.map(iface => {
        ...
        throw new Error(`For type ${this.getTypeName()} you provide incorrect interface object ${inspect(iface)}`);
      });
    };
  ...
  getFieldExtensions(fieldName) {
    ...
    if (isObject(field) && !isFunction(field) && !Array.isArray(field) && !isComposeOutputType(field)) {
